 ...
 *  - `gzipRoot`    Root directory for cached gzip files, defaults to root
 *
 * @param {String} root
...
exports = module.exports = function staticGzip(root, options) {
  var options = options || {},
      ...
      gzipRoot = options.gzipRoot || root;
  ...
  return function(req, res, next) {
    ...
    if (!matchType.test(type)) {
      return passToStatic(filename, root);
    }
    ...
    fs.stat(filename, function(err, stat) {
      ...
      var base = path.basename(filename),
          ...
          gzipname = path.join(gzipRoot, pathname + '.gz');
      fs.stat(gzipname, function(err, gzStat) {
        // truncate milliseconds to work around node.js bug:
        // https://github.com/nodejs/node-v0.x-archive/issues/9369
        if (err && err.code === 'ENOENT' || trunc(+gzStat.mtime / 1000) !== trunc(+stat.mtime / 1000)) {
          mkdirp(path.dirname(gzipname), function(err) {
            if (err) return passToStatic(filename, root);
			...
			outfile.on('close', function() {
				...
				fs.utimes(tmpname, stat.mtime, stat.mtime, function() {
					// date modified set to match original file
      ...
      function sendGzip() {
        ...
        passToStatic(gzipname, gzipRoot);
      }
    });

	// send file
    function passToStatic(name, root) {
      send(req, name.substr(root.length), {
        root: root,
